...
**레거시 ERP가 고객 신원을 쥐고 있는 상황에서의 통합회원 연동.** 국내 아웃도어 기어 리테일러로, 온라인 스토어보다 오프라인 멤버십이 훨씬 먼저 있었습니다. 회원·등급·포인트 잔액은 전부 ERP에 있고, 그 ERP는 스토어보다 오래됐고 아마 더 오래 갈 겁니다.

온라인 스토어를 열자 15년 단골이 포인트 0인 신규 고객으로 들어왔습니다. 두 시스템은 그게 같은 사람인 줄 몰랐습니다.

**어려웠던 지점**

- **신원의 소유권이 밖에 있습니다.** 멤버십의 원본은 ERP, 주문의 원본은 커머스 플랫폼이고, 어느 쪽도 상대에게 양보시킬 수 없습니다. ERP는 커머스 어드민을 평생 안 열어볼 매장 직원들이 쓰고, 플랫폼에는 이 멤버십 체계를 표현할 기본 자료구조가 없습니다. 둘을 잇는 유일하게 신뢰할 만한 키는 전화번호였고, 이는 **본인이 그 번호를 실제로 보유했음을 증명하지 않으면 연결할 수 없다**는 뜻입니다.
- **포인트는 결제에서 써져야 합니다.** 조회만 되는 잔액은 아무도 안 씁니다. 차감은 플랫폼이 관장하는 체크아웃 안에서 일어나야 하고, 잔액은 결제 그 시점에 정확해야 합니다.
- **스토어프론트가 이걸 끊임없이 읽습니다.** 등급 뱃지·포인트 잔액·회원가가 거의 모든 페이지에 렌더됩니다. 렌더마다 ERP 왕복이 필요한 설계는 첫 트래픽 스파이크에서 무너집니다.

**핵심 결정**

- **서비스는 하나, 새 기능은 그 안의 패키지로.** [케이스 01](01-referral-reward-backend.md)과 같은 Go 보일러플레이트를 쓰면서 배포 결정은 정반대로 갔습니다. 01은 보상 계산·인출·포인트샵이 변경 주기도 장애 반경도 실제로 달랐지만, 여기서는 인증·연동·포인트·마이페이지가 전부 "이 고객의 멤버십"이라는 한 가지의 다른 면일 뿐입니다. 이걸 쪼갰다면 **항상 같이 배포되고 항상 같이 죽는 서비스들** — 마이크로서비스의 비용만 있고 이득은 없는 형태 — 이 나왔을 겁니다.
- **메타필드는 캐시이고, 캐시로 다룹니다.** 등급을 플랫폼 고객 객체에 써두면 스토어프론트가 회원 상태를 공짜로 렌더합니다. 규율은 하나 — **메타필드로는 아무것도 판단하지 않습니다.** 적립·차감·자격 판정은 전부 서비스 자체 DB를 읽고, 메타필드는 표시 전용입니다.
- **여기선 자체 JWT가 아니라 App Proxy 서명.** 01은 트리 위치·파트너 여부처럼 플랫폼 고객 모델이 못 담는 구조가 있어서 자체 토큰을 발급했습니다. 여기서는 연결된 순간 **플랫폼 고객이 곧 신원**이라 따로 표현할 게 없었습니다. App Proxy 서명 검증을 택해 비밀번호 저장도, 세션 관리도, 고객의 2차 로그인도 없앴습니다.
- **문자 인증은 전환율을 깎아도 뺄 수 없습니다.** 잘못 연결되면 남의 구매 이력과 포인트에 접근하게 됩니다. 미인증 번호 매칭은 그대로 계정 탈취 경로입니다.
- **계약은 OpenAPI로.** 테마·체크아웃 익스텐션·ERP 연동을 서로 다른 사람이 다른 시점에 붙였습니다. 스펙을 코드와 함께 커밋하고 실행 중인 서비스가 직접 서빙하게 해서 계약과 구현이 조용히 어긋나지 않게 했습니다.

**결과**

- 오프라인 단골이 온라인에서 등급·포인트를 그대로 갖고 시작 (자기 신고가 아니라 인증된 문자 챌린지 기반)
- 포인트가 마이페이지 조회용이 아니라 체크아웃에서 실제 차감
- 페이지별 회원 상태는 플랫폼 메타필드에서 렌더 — 스토어프론트 핫패스에 서비스·ERP 호출 없음
- 마이페이지 주문·프로필·체크아웃 포인트를 한 서비스의 패키지로 추가 — 기능마다 새 배포·새 DB·새 ingress 규칙 없음
