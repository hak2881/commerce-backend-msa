...
미국 뷰티 디바이스 브랜드의 **추천 보상 백엔드**. 회원이 회원을 데려오고, 한번 맺어진 그 관계는 바뀌지 않습니다. 이후 모든 주문의 적립 대상이 여기서 정해집니다. 그 위에 크레딧을 현금으로 뽑을 수 있는 Partner 등급이 따로 있습니다. 커머스는 Shopify Plus 단일 스토어를 씁니다(B2B는 네이티브 컴퍼니 계정, B2C는 같은 스토어의 다른 도메인). 적립 규칙은 전부 플랫폼 바깥에 있습니다.

**어려웠던 지점**

1. **보상 규칙이 계속 바뀌었습니다.** 트리 적립(본인과 업라인 2단, 각자 자기 등급률 적용), Retail QR 적립(트리로 퍼지지 않고 1명), Partner 적립이 각각 따로 있었습니다. 요율도 대상도, 심지어 원장을 몇 개 둘지까지 여러 번 뒤집혔습니다.
2. **틀리면 안 되는 돈.** 크레딧은 Partner가 현금으로 인출합니다. 반올림 오차는 화면이 잘못 나온 게 아니라 지급액이 어긋난 겁니다. 포인트가 소수점 2자리라, float를 쓰면 딱 잔액이 안 맞기 시작하는 구간에 들어갑니다.
3. **전부 at-least-once.** 웹훅은 재전송됩니다. 재전송된 배송 웹훅이 적립을 한 번 더 돌리면 세 사람에게 두 번씩 지급됩니다.

**핵심 결정**

- **보상 계산은 `reward` 서비스에만 둡니다.** 요율도 조건도 등급 기준도 다른 곳에는 하드코딩하지 않았습니다. 정책이 계속 흔들려도 고칠 곳이 한 군데라, 이 결정 하나로 버텼습니다. `order`나 `partner`가 "자기 몫"을 계산하게 했다면 정책이 바뀔 때마다 코드베이스 네 개를 손봐야 했을 겁니다.
- **원장 1개, 인출은 권한으로.** 처음엔 스토어 크레딧과 현금화 포인트, 원장이 두 개였습니다. "누가 현금화할 수 있는가"는 정책 문제인데 그걸 데이터 모델에 새겨둔 셈이라, 규칙이 바뀔 때마다 데이터를 옮겨야 했고 잔액 조회는 매번 두 테이블을 합쳐야 했습니다. 단일 `credit_ledger`로 합치고, 인출 가능 여부는 파트너 권한만 보고 판단하도록 바꿨습니다. 현금화는 어디에 저장하느냐가 아니라 누구에게 허용하느냐의 문제입니다.
- **전부 정수로.** 금액은 센트 단위 `int64`, 포인트는 ×100 한 `int64`입니다(3.33pt는 333으로 저장). 변환은 화면에 내보낼 때만 하고, 적립 경로에서는 float를 건드리지 않습니다.
- **경계 6개, 배포 1개.** 처음엔 6개 서비스로 설계하고 그대로 배포했다가 나중에 단일 바이너리로 합쳤습니다. 대신 패키지 분리, 논리 DB 분리, 교차 조회 금지, 내부 API Key 호출은 그대로 뒀습니다. 트래픽이 파드 6개와 커넥션 풀 6벌을 감당할 만큼은 아니었을 뿐이고, 경계 자체는 지킬 만했습니다. 다시 쪼개는 일은 재작성이 아니라 배포 방식만 바꾸면 됩니다.
- **게이트웨이는 지웠습니다.** 원래 설계에는 앞단에 게이트웨이 서비스가 있었는데, 스토어프론트 인증이 서비스별 JWT 검증으로 옮겨간 뒤로는 홉 하나와 장애 지점 하나만 늘리고 있었습니다.

**결과**

- 배포를 6개에서 1개로 합치면서 서비스 코드는 손대지 않고 경계 6개를 유지 (조립과 라우팅만 변경)
- 원장 2개를 1개로 통합하고 과거 잔액 이관, 인출 자격은 권한 체크 한 번으로 축소
- JWT 도입 후 게이트웨이와 App Proxy 인증을 모두 걷어내 스토어프론트 요청 경로에서 홉 2개 제거
- 적립 멱등성을 세 겹으로 확보: 웹훅 전달 ID, 원장 idem_key, outbox 드레인

**다시 한다면** — outbox 드레인이 폴링 방식입니다. 지금 주문량에서는 문제없고 장애가 났을 때 동작도 단순하지만, 적립 지연 시간에 하한이 생깁니다. 리스너 방식이면 그 하한이 없겠죠. 적립이 고객에게 실시간으로 보여야 할 때나 바꿀 일이고, 지금은 결제가 아니라 배송 시점에 적립되니 해당되지 않습니다.
