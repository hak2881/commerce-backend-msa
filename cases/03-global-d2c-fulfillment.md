...
국내 대시캠 제조사의 **글로벌 D2C 백엔드**. 딜러 채널과 소비자 채널로 스토어프론트가 두 개 있고, 그 뒤에 시간을 두고 쌓인 백엔드가 세 개 있었습니다. 소비자 쪽에는 예전 스토어프론트였던 WordPress가 고객 데이터를 그대로 쥔 채 남아 있었습니다. 중간에 넘겨받아 확장했고, 마지막에는 고객사 팀이 직접 운영할 수 있게 패키징했습니다.

**어려웠던 지점**

- **채널 둘, 카탈로그 하나, 나머지는 전부 다름.** 딜러와 소비자는 상품만 공유하고 가격과 등록, 배송이 다 다릅니다. 스토어프론트와 백엔드를 나눈 건 타당한 선택이었지만, 대신 카탈로그 싱크나 플랫폼 API 클라이언트, 업로드 처리 같은 공통 관심사가 두 벌씩 존재하고 서로 어긋날 여지가 생깁니다.
- **국제 배송비는 조회 테이블로 안 됩니다.** 목적지와 중량, 부피, 택배사 실시간 요금에 따라 달라지고, 고객이 결제를 확정하기 **전에** 견적이 나와야 합니다. 서드파티 API가 구매 경로의 임계 경로에 들어온다는 뜻입니다.
- **레거시 CMS가 여전히 고객을 갖고 있었습니다.** 소비자 쪽에는 몇 년치 WordPress 계정과 콘텐츠가 쌓여 있었습니다. 스크립트 한 번 돌려서 끝날 일이 아니라, 이관하는 동안 두 시스템이 나란히 살아 있어야 했습니다.
- **장기 실행 작업과 요청 처리가 런타임을 공유**하고 있었습니다. 소비자 백엔드가 카탈로그·고객 싱크를 API와 같은 서버리스 함수에서 돌렸습니다.

**핵심 결정**

- **배송 서비스만 Lambda에 올리지 않았습니다.** 요금 견적은 구매 흐름 한가운데 있습니다. 고객이 화면을 보며 기다리는 중인데 콜드 스타트에 택배사 왕복까지 겹치면, 하필 가장 나쁜 타이밍에 눈에 띄게 멈춥니다. 그래서 ALB 뒤에 워커 수를 고정한 컨테이너로 상시 띄워둡니다. 나머지 둘은 딜러 로그인과 관리 작업 사이에 대부분 놀고 있어서, 웜 레이턴시보다 scale-to-zero가 남는 장사입니다. 런타임은 조직 표준이 아니라 워크로드가 지연에 얼마나 민감한지를 보고 고릅니다.
- **읽기/쓰기 분리는 애플리케이션 코드가 아니라 ORM 라우터에서.** Django 두 서비스 모두 DB 라우터로 쓰기는 프라이머리에, 읽기는 리플리카에 보냅니다. 뷰든 시리얼라이저든 태스크든 어느 커넥션인지 기억할 필요가 없고, 나중에 합류한 사람이 잊을 수도 없습니다.
- **정말로 들쭉날쭉한 워크로드라서 Aurora Serverless.** 글로벌 D2C 트래픽은 시간대와 신제품 출시를 따라 움직입니다. 피크에 맞춘 인스턴스는 하루 대부분을 놀고, 중앙값에 맞추면 출시일에 넘어갑니다.
- **경계에서는 camelCase, 안에서는 snake_case.** 이 서비스들을 부르는 쪽은 스토어프론트 JavaScript입니다. 프론트마다 키를 변환하게 하거나 반대로 파이썬 코드가 JS 관례를 따르게 하는 대신, 변환은 엣지의 시리얼라이저 한 곳에서만 합니다. 경계 양쪽이 각자의 언어에서 자연스럽게 읽힙니다.
- **소비자 백엔드의 900초 타임아웃은 다시 하지 않을 타협입니다.** 카탈로그 정합과 레거시 고객 이관 작업이 API와 같은 함수에서 도는 바람에, API에 필요한 값의 30배짜리 타임아웃을 함수 전체에 걸어야 했습니다. 돌아가긴 했고 이관 기간에 배포 표면을 작게 유지해준 것도 사실입니다. 하지만 싱크가 폭주하면 요청을 처리하는 동시성 풀을 그대로 잡아먹고, 타임아웃은 API 경로의 안전장치 역할을 못 합니다. 이 작업들은 스케줄 트리거 함수나 큐 워커로 빠져야 합니다.
- **인계도 결과물로 봤습니다.** 고객사 팀이 저 없이 운영할 수 있어야 했으니까요. 인프라 아키텍처와 DB 스키마를 문서로 남기고, AWS 식별자(계정 ID, RDS 엔드포인트, ACM ARN, VPC/서브넷, ECR URI)는 전부 placeholder로 바꿔 별도 시트로 분리했습니다. 체크섬을 붙인 소스 번들과 환경변수 레퍼런스도 같이 넘겼습니다. 계정 ID가 박힌 문서는 누구에게도 건넬 수 없는 문서입니다.

**결과**

- 서비스 3개를 문서화된 하나의 인프라 모델로 정리하고, 런타임 선택 근거를 물려받은 관행이 아니라 트래픽 형태로 명시
- 소비자 구매 경로에서 실시간 택배사 요금 견적 제공, 콜드 함수가 아니라 워밍된 컨테이너에서 서빙
- 레거시 CMS 고객과 콘텐츠를 두 시스템이 동시에 살아 있는 상태로 커머스 플랫폼에 이관
- 인프라 아키텍처, DB 스키마, 환경변수 레퍼런스, 체크섬 소스 번들로 구성된 인계 패키지 완성 (계정 종속 식별자는 전부 파라미터화)
